  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <article className="bg-white/[0.03] border border-white/8 rounded-2xl p-6 mb-6">
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-1 text-sm text-white/40 flex-shrink-0">
            <MessageSquare className="w-4 h-4" /><span>{post.reply_count}</span>
          </div>
        </div>
        <p className="text-white/75 leading-relaxed whitespace-pre-wrap text-sm">{post.content}</p>
      ...
      <div className="mb-6">
        ...
        {replies.length === 0 && !post.is_locked && (
        ...
        <div className="space-y-3">
          {replies.map((reply, i) => (
            <div
              key={reply.id}
              className={`bg-white/[0.03] border rounded-xl p-5 ${reply.is_solution ? 'border-teal-400/30 bg-teal-400/5' : 'border-white/8'}`}
            >
              {reply.is_solution && (
              ...
              <p className="text-white/75 text-sm leading-relaxed whitespace-pre-wrap">{reply.content}</p>
      ...
      {post.is_locked ? (
        <div className="bg-white/3 border border-white/8 rounded-xl px-5 py-4 text-center">
          ...
          <p className="text-white/40 text-sm">This post is locked. No new replies allowed.</p>
        </div>
      ) : (
        <div className="bg-white/[0.03] border border-white/8 rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-white/70 mb-4">Add a Reply</h3>
          {replyError && (
            <div className="bg-red-400/10 border border-red-400/20 rounded-lg p-3 text-red-300 text-sm mb-3">{replyError}</div>
          )}
          ...
          <p className="mt-3 text-xs text-white/25 leading-relaxed">
            Replies are personal experiences only — not legal advice.
          </p>
        </div>
      )}
    </div>
