...
import {
  ...
  MapPin, MessageSquare, Users, Plus, TrendingUp, Loader2
} from 'lucide-react'
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="bg-yellow-400/5 border border-yellow-400/15 rounded-xl px-4 py-3 mb-8">
        <p className="text-yellow-300/80 text-xs leading-relaxed">
          <span className="font-semibold text-yellow-300">Community disclaimer:</span> Posts are personal experiences and opinions shared by members. Nothing here constitutes legal advice. Consult a licensed attorney for advice specific to your situation.
        </p>
      </div>
      ...
      {error && (
        <div className="bg-red-400/10 border border-red-400/20 rounded-xl p-4 text-red-300 text-sm">{error}</div>
      )}
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {categories.map(cat => {
            ...
            const colorClass = COLOR_CLASS[cat.color] ?? COLOR_CLASS.gray
            const dotClass = DOT_CLASS[cat.color] ?? DOT_CLASS.gray
            return (
              <Link
                key={cat.id}
                href={`/forum/${cat.slug}`}
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-white/50 leading-relaxed line-clamp-2">{cat.description}</p>
                  </div>
                </div>
                <div className="mt-3 flex items-center gap-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${dotClass}`} />
                  <span className="text-xs text-white/30 group-hover:text-white/50 transition-colors">Browse posts →</span>
                </div>
              </Link>
            )
          })}
