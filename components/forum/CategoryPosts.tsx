...
export function CategoryPosts({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/forum/posts?category=${slug}&page=${page}`)
      ...
      .then(data => {
        if (data) { setPosts(data.posts ?? []); setTotal(data.total ?? 0) }
      })
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {!loading && !error && posts.length === 0 && (
        <div className="text-center py-16">
          ...
          <Link href="/forum/new" className="mt-4 inline-block text-teal-400 hover:text-teal-300 text-sm">Create a post →</Link>
        </div>
      )}
      {!loading && !error && posts.length > 0 && (
            ...
            >
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-white/40 line-clamp-2 leading-relaxed">{post.content}</p>
                </div>
